...
class TrivialisedDiffusionDev(nn.Module):
    ...
    - internal wrapped chart scaled by π
    ...
    def __init__(
        ...
        # Internal wrapped chart period. External/public positions remain in [0,1)
        # and signed displacements remain in [-0.5, 0.5).
        self.scale_pos = math.pi
    ...
    def forward_sample(
        ...
        if epsilon_v is None:
            ...
            epsilon_v = epsilon_v.to(dtype=v0_internal.dtype, device=v0_internal.device)
        ...
        mu_r_t = self.wrapped_gaussian_mu_r_t(t, v_t_internal, v0_internal)
        sigma_r_t = self._match_dims(self.wrapped_gaussian_sigma_r_t(t), f0_internal)

        if epsilon_r is None:
            epsilon_r = torch.randn_like(f0_internal)
        else:
            epsilon_r = epsilon_r.to(dtype=f0_internal.dtype, device=f0_internal.device)
        epsilon_r = scatter_center(epsilon_r, index=index)

        r_t_internal = self._wrap_internal(mu_r_t + sigma_r_t * epsilon_r)
    ...
    def score_target(
        ...
        mu_r_t = self._wrap_internal(self.wrapped_gaussian_mu_r_t(t, v_t_internal, v0_internal))
