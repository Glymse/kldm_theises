...
class TrivialisedDiffusion(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("_log_sigma_r_grid", log_sigma_r)
    ...
    def _interp_t01_from_log_sigma_r(self, log_sigma_r: torch.Tensor) -> torch.Tensor:
        ...
        grid_log_sigma = self._log_sigma_r_grid.to(device=log_sigma_r.device, dtype=log_sigma_r.dtype)

        clamped = log_sigma_r.clamp(min=grid_log_sigma[0], max=grid_log_sigma[-1])
        idx_hi = torch.searchsorted(grid_log_sigma, clamped, right=False)
        idx_hi = idx_hi.clamp(1, grid_log_sigma.numel() - 1)
        idx_lo = idx_hi - 1

        log_lo = grid_log_sigma[idx_lo]
        log_hi = grid_log_sigma[idx_hi]
        t_lo = grid_t01[idx_lo]
        t_hi = grid_t01[idx_hi]
        frac = (clamped - log_lo) / (log_hi - log_lo).clamp_min(self.eps)
        return t_lo + frac * (t_hi - t_lo)

    def _interp_log_sigma_r_from_t01(self, t01: torch.Tensor) -> torch.Tensor:
        grid_t01 = self._log_sigma_t01_grid.to(device=t01.device, dtype=t01.dtype)
        grid_log_sigma = self._log_sigma_r_grid.to(device=t01.device, dtype=t01.dtype)

        clamped = t01.clamp(min=grid_t01[0], max=grid_t01[-1])
        idx_hi = torch.searchsorted(grid_t01, clamped, right=False)
        idx_hi = idx_hi.clamp(1, grid_t01.numel() - 1)
        idx_lo = idx_hi - 1
        ...
        log_lo = grid_log_sigma[idx_lo]
        log_hi = grid_log_sigma[idx_hi]
        frac = (clamped - t_lo) / (t_hi - t_lo).clamp_min(self.eps)
        return log_lo + frac * (log_hi - log_lo)
    ...
    def sample_t01(
        ...
        log_sigma_max = self._log_sigma_r_grid[-1].to(device=device, dtype=log_sigma_min.dtype)
        u = torch.rand(size, device=device, dtype=log_sigma_min.dtype)
        log_sigma = log_sigma_min + (log_sigma_max - log_sigma_min) * u
        return self._interp_t01_from_log_sigma_r(log_sigma)
    ...
    def sampling_t01_schedule(
        ...
        log_sigma_min = self._interp_log_sigma_r_from_t01(lb_t01)
        log_sigma_max = self._log_sigma_r_grid[-1].to(device=device, dtype=log_sigma_min.dtype)
        weights = torch.linspace(0.0, 1.0, n_steps, device=device, dtype=log_sigma_min.dtype)
        log_sigma_schedule = log_sigma_max + (log_sigma_min - log_sigma_max) * weights
        t01_schedule = self._interp_t01_from_log_sigma_r(log_sigma_schedule)
        return torch.cat([t01_schedule, t01_schedule.new_tensor([0.0])], dim=0)
